...
- **Single Responsibility Principle (SRP)**은 클래스나 모듈을 변경할 이유가 단 하나 뿐이어야 한다는 원칙이다. (175)
- 문제는 우리들 대다수가 프로그램이 돌아가면 일이 끝났다고 여기는 데 있다. **'깨끗하고 체계적인 소프트웨어'**라는 **다음 관심사**로 전환하지 않는다. 만능 클래스를 단일 책임 클래스 여럿으로 분리하는 대신 다음 문제로 넘어가버린다. (176)
